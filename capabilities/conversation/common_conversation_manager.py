...
class CommonConversationManager(IConversationManagerCapability):
    ...
    def initialize(self, config: Dict[str, Any] = None):
        ...
        # 1. 如果 DraftRepository 支持配置 (比如过期时间)，在这里设置
        if hasattr(self.draft_repo, 'configure'):
            repo_config = {
                "max_items": config.get("max_drafts_per_user", 5),
                "expiration_hours": config.get("draft_expiration_hours", 24)
            }
            ##TODO:加入过期配置
            # self.draft_repo.configure(repo_config)
            
        # --- B. 初始化依赖项 ---
        
        # 解决循环导入：在方法内部导入 Registry
    ...
    def handle_user_input(self, user_input: str, user_id: str = "default_user") -> Dict[str, Any]:
        ...
        from ..intent_router.interface import IIntentRouterCapability # 假设路径
    ...
    def process_user_input(self, user_input: str, user_id: str = "default_user") -> str:
        ...
        from capabilities.intent_router.interface import IIntentRouterCapability
        intent_router = capability_registry.get_capability("intent_router", IIntentRouterCapability)
