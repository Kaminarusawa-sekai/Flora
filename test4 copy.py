...
CSV_FILE = "market_tree.csv"         # CSV 文件路径（需在当前目录或指定绝对路径）
...
def main():
    ...
    with open(CSV_FILE, 'r', encoding='utf-8') as f:
        ...
        for row in tqdm(reader, total=total_rows, desc="🔄 更新节点"):
            ...
            print(f"Processing row: code={repr(code)}, dify={repr(dify)}")
            batch.append((code,database, dify))
...
def update_batch(driver, batch):
    ...
    params = {
        "batch": [
            {"code": code,"database":db,"dify":dify}
            for code,db,dify in batch
        ]
    }
