...
class ObserverService:
    ...
    # 重构查询方法，使用传入的 session
    async def get_task_instance(self, session: AsyncSession, task_id: str) -> Optional[TaskInstance]:
        inst_repo = create_task_instance_repo(session, dialect)
        return await inst_repo.get(task_id)

    async def get_trace_instances(self, session: AsyncSession, trace_id: str) -> List[TaskInstance]:
        inst_repo = create_task_instance_repo(session, dialect)
        return await inst_repo.find_by_trace_id(trace_id)

    async def get_trace_status_summary(self, session: AsyncSession, trace_id: str) -> dict:
        inst_repo = create_task_instance_repo(session, dialect)
    ...
    async def get_ready_tasks(self, session: AsyncSession) -> List[TaskInstance]:
        inst_repo = create_task_instance_repo(session, dialect)
