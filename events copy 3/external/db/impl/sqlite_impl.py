        ...
        stmt = (
            ...
            .where(
                and_(
                    ...
                    TaskInstanceDB.node_path.like(path_pattern)
                )
            )
...
class SQLiteTaskDefinitionRepository(TaskDefinitionRepository):
    def __init__(self, session: AsyncSession):
        self.session = session
    ...
    async def get(self, def_id: str) -> TaskDefinition:
        ...
        logger.info(f"Getting task definition {def_id} from session {self.session}")
        result = await self.session.execute(stmt)
    ...
    async def create(self, definition: TaskDefinition) -> None:
        db_definition = TaskDefinitionDB(
            id=definition.id,
