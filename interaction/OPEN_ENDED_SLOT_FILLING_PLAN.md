...
**修改内容**：
- 保留实体填充逻辑（不要使用NLU，直接使用llm进行实体填充）
- 新增动态评估步骤
- 根据评估结果更新Draft状态
- 生成合适的回复文本

**核心流程**：
1. 实体填充：将识别到的实体填入槽位（不要使用NLU，直接使用llm进行填充）
2. 动态评估：调用LLM判断任务完整性
...
-在 TaskDraftDTO 中，你已经加了 next_clarification_question，这很好！ 确保在下一轮调用 _evaluate_draft_completeness 时，把 draft 里存的 last_system_question (即上一轮的 clarification) 显式地加到 Prompt 里。
...
**错误处理策略**：
- LLM调用失败时，使用兜底逻辑
- 兜底逻辑：默认认为任务未完成，请求用户提供更多信息（策略 A (保守)：直接回复：“抱歉，我刚刚走神了，没听清您的要求，能麻烦您再详细描述一遍任务吗？”（不推进状态，单纯让用户重试）。）
- 记录错误日志，便于后续分析
